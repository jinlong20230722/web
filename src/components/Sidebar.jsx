  ...
  return <div className={`bg-[#1E3A8A] text-white transition-all duration-300 ${collapsed ? 'w-16' : 'w-64'} min-h-screen flex flex-col shadow-xl`}>
      ...
      <div className="p-4 border-b border-blue-800">
        <div className="flex items-center justify-between">
          {!collapsed && <div>
              <h1 className="font-bold tracking-wide text-[1.8rem]">天顺保安</h1>
      ...
      <nav className="flex-1 p-3 overflow-y-auto">
        <ul className="space-y-1">
          ...
          return <li key={item.id}>
                <button onClick={() => onPageChange(item.id)} className={`w-[120px] h-[40px] flex items-center gap-3 px-3 rounded-lg transition-all duration-200 relative whitespace-nowrap overflow-hidden ${isActive ? 'bg-blue-600 text-white shadow-lg' : 'text-blue-100 hover:bg-blue-800 hover:text-white'}`} title={collapsed ? `${item.label} - ${item.description}` : ''}>
